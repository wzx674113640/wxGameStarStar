cc.Class({
    ...
    // 列表初始化
    initialize: function () {
        this.worldData = this.node.parent.getComponent("GameControl").worldData;
        this.totalCount = this.worldData.length;
        ...
        for (let i = 0; i < this.spawnCount; ++i) { // spawn items, we only need to do this once
            ...
            item.getComponent('RankItem').init(i,obj[i]);
    	}
    },
        ...
        for (let i = 0; i < items.length; ++i) {
            ...
            if (isDown) {
                ...
                if (viewPos.y < -this.bufferZone && newY < 0) {
                    ...
                    var obj = this.worldData;
                    item.getComponent('RankItem').init(itemId,obj[itemId]);
                }
            } else {
                // 提前计算出该item的新的y坐标
                newY = items[i].y - offset;
